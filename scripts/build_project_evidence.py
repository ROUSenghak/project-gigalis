...
def data_quality_profile(cohort: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "benchmark_provenance": {
            ...
            "interpretation": (
                "A stratified Grand Ouest review carried out against real BOAMP "
                "notices before the linkage methods existed. Independent of those "
                "methods, but a single-pass LLM-assisted review rather than an "
                "independent specialist panel, with corpus-relative negatives."
            ),
...
**5. Reference design.** A stratified review of `{reference['reviewed_anchors']}` Grand
Ouest anchors against real BOAMP notices on `{reference['review_date']}`, before these
linkage methods existed. `{reference['remap']['resolved_to_current_episodes']}` re-resolve
onto the current episode reconstruction. See `REGIONAL_BENCHMARK_REFERENCE.md`.
...
because the reviewer saw roughly 25 candidates per anchor rather than the full
...
The main risks are measurement rather than pipeline corruption. Validated SIREN is missing for `{missing['buyer_siren']:.1%}` of the cohort, reliable duration for `{missing['reliable_duration']:.1%}`, and the reference labels are a single-pass LLM-assisted review rather than independent specialist judgements. Therefore the linkage and survival results are usable as conservative exploratory evidence, but their reported precision is not an independently established accuracy guarantee.
...
Two limits remain. The labels are a single-pass LLM-assisted review, not an independent specialist panel, and negatives are corpus-relative because the reviewer saw roughly 25 candidates per anchor. Recall is additionally capped at `{profile['benchmark_provenance']['candidate_generation_recall_ceiling']:.3f}` by candidate generation, before any method runs.
